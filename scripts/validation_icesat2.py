# -*- coding: utf-8 -*-
"""
DEM Validation vs. ICESat-2 Python Script
...
dataset_batch = np.array(['NASADEM_srtm', 'COP', 'SRTMV3'])

version_str = 'v9_2023_10_19'
icesat_str = 'bareshots' # set to 'bareshots' or 'vegshots' depending on whether you want bare earth or vegetated results
figure_str = 'run1'


icesat2_path = os.path.join(root_path, 'icesat2_nasadem_atl08_aggregated/')
...
### PROCESSING OPTIONS ###
##########################
overwrite_icesat_tile_stats = False # overwrite tile statistics files if already exist
overwrite_hansen_mask = False # overwrite hansen forest cover mask for each NASADEM tile if already exist
overwrite_glims_mask = False # overwrite GLIMS mask data
skip_plots = False
plot_global = True

percentile_threshold = [1, 99] # percentile outlier thresholds (applied to raster - ICESat height differences)

# List of continents to process (NASADEM tiles and ICESat-2 data must be sorted into these subfolders within the paths specified above).
...
def get_tile_level_stats(nasadem_file_pattern, dem_dataset, icesat_path, forested_flag=False, max_slope_dem=5.0,
                         min_forest_cover=0, max_forest_cover=10, min_h_canopy=0, max_h_canopy=5, max_toc_roughness=1,
                         max_te_uncertainty=1, min_segment_photons=150, max_msw_flag=2, min_snr=1.1,
                         smart_orbit_filter=False, min_tile_samples=50,
                         overwrite_hansen_mask=False, overwrite_glims_mask=False,
                         nasadem_type='merged'):
    """ Calculate tile-level statistics between NASADEM and ICESat-2.
    ...
    hcanopy_vector = np.array([], dtype='float32')
    ...
    if not os.path.isdir(os.path.join(outpath, continent+'/hansen_masks/')):
        os.makedirs(os.path.join(outpath, continent+'/hansen_masks/'))
    if not os.path.isdir(os.path.join(outpath, continent+'/glims_masks/')):
        os.makedirs(os.path.join(outpath, continent+'/glims_masks/'))
    ...
    for i, file in enumerate(nasadem_glob):
        ...
        icesat_geojson_gzip_file = os.path.join(icesat_path, nasadem_root_name+'.geojson.gzip')

        if os.path.isfile(icesat_geojson_gzip_file):
            icesat_geojson = read_geojson_gzip(icesat_geojson_gzip_file)

            icesat_lon = np.array(icesat_geojson['geometry'].x, dtype='float64')
            icesat_lat = np.array(icesat_geojson['geometry'].y, dtype='float64')
            icesat_ground = np.array(icesat_geojson['h_te_best_fit'], dtype='float32')
            icesat_h_canopy = np.array(icesat_geojson['h_canopy'], dtype='float32')
            icesat_rh50 = np.array(icesat_geojson['canopy_h_metrics_50'], dtype='float32')
            icesat_rh75 = np.array(icesat_geojson['canopy_h_metrics_75'], dtype='float32')
            icesat_toc_roughness = np.array(icesat_geojson['toc_roughness'], dtype='float32')
            icesat_ground_uncertainty = np.array(icesat_geojson['h_te_uncertainty'], dtype='float32')
            icesat_forest_cover = np.array(icesat_geojson['hansen_treecover_2000'], dtype='float32')
            icesat_hansen_gain = np.array(icesat_geojson['hansen_gain'], dtype='float32')
            icesat_hansen_loss = np.array(icesat_geojson['hansen_lossyear'], dtype='float32')
            icesat_landcover = np.array(icesat_geojson['segment_landcover'], dtype='float32')
            icesat_snowcover = np.array(icesat_geojson['segment_snowcover'], dtype='float32')
            icesat_photons = np.array(icesat_geojson['n_seg_ph'], dtype='float32')
            icesat_msw = np.array(icesat_geojson['msw_flag'], dtype='float32')
            icesat_layer_flag = np.array(icesat_geojson['layer_flag'], dtype='float32')
            icesat_pekel = np.array(icesat_geojson['peckel_occurrence'], dtype='float32')
            icesat_snr = np.array(icesat_geojson['snr'], dtype='float32')
            icesat_glims = np.array(icesat_geojson['glims_buffered'], dtype='float32')
            icesat_end_orbit = np.array(icesat_geojson['end_orbit'], dtype='float32')
            icesat_canopy_openness = np.array(icesat_geojson['canopy_openness'], dtype='float32')
            icesat_canopy_photons = np.array(icesat_geojson['n_ca_photons'], dtype='float32')

            ind_nan_rh50 = np.isnan(icesat_rh50) # replace NaNs (points where canopy flag is not set) with zeros
            if np.any(ind_nan_rh50):
                icesat_rh50[ind_nan_rh50] = 0.0

            ind_nan_rh75 = np.isnan(icesat_rh75) # replace NaNs (points where canopy flag is not set) with zeros
            if np.any(ind_nan_rh75):
                icesat_rh75[ind_nan_rh75] = 0.0

            ind_nan_h_canopy = np.isnan(icesat_h_canopy) # replace NaNs (points where canopy flag is not set) with zeros
            if np.any(ind_nan_h_canopy):
                icesat_h_canopy[ind_nan_h_canopy] = 0.0

            ind_nan_toc_roughness = np.isnan(icesat_toc_roughness) # replace NaNs (points where canopy flag is not set) with zeros
            if np.any(ind_nan_toc_roughness):
                icesat_toc_roughness[ind_nan_toc_roughness] = 0.0

            ind_nan_uncertainty = ~np.isfinite(icesat_ground_uncertainty)
            if np.any(ind_nan_uncertainty):
                icesat_ground_uncertainty[ind_nan_uncertainty] = 1e10

            ind_nan_landsat_forest_cover = ~np.isfinite(icesat_forest_cover)
            if np.any(ind_nan_landsat_forest_cover):
                icesat_forest_cover[ind_nan_landsat_forest_cover] = 0
            icesat_forest_cover = icesat_forest_cover.astype('int32')

            ind_nan_landcover = ~np.isfinite(icesat_landcover)
            if np.any(ind_nan_landcover):
                icesat_landcover[ind_nan_landcover] = 1
            icesat_landcover = icesat_landcover.astype('int32')

            ind_nan_snowcover = ~np.isfinite(icesat_snowcover)
            if np.any(ind_nan_snowcover):
                icesat_snowcover[ind_nan_snowcover] = 1
            icesat_snowcover = icesat_snowcover.astype('int32')

            ind_nan_photons = ~np.isfinite(icesat_photons)
            if np.any(ind_nan_photons):
                icesat_photons[ind_nan_photons] = 0
            icesat_photons = icesat_photons.astype('int32')

            ind_nan_msw = ~np.isfinite(icesat_msw)
            if np.any(ind_nan_msw):
                icesat_msw[ind_nan_msw] = 5
            icesat_msw = icesat_msw.astype('int32')

            ind_nan_layer = ~np.isfinite(icesat_layer_flag)
            if np.any(ind_nan_layer):
                icesat_pekel[ind_nan_layer] = 0
            icesat_layer_flag = icesat_layer_flag.astype('int32')

            ind_nan_pekel = ~np.isfinite(icesat_pekel)
            if np.any(ind_nan_pekel):
                icesat_pekel[ind_nan_pekel] = 100
            icesat_pekel = icesat_pekel.astype('int32')

            ind_nan_snr = ~np.isfinite(icesat_snr)
            if np.any(ind_nan_snr):
                icesat_snr[ind_nan_snr] = 0

            ind_nan_end_orbit = ~np.isfinite(icesat_end_orbit)
            if np.any(ind_nan_end_orbit):
                icesat_end_orbit[ind_nan_end_orbit] = -1
            icesat_end_orbit = icesat_end_orbit.astype('int32')

            # check if there's no data in this geojson file, in which case, skip it
            if (len(icesat_lon) <= 1) or (len(icesat_lat) <= 1) or (len(icesat_ground) <= 1) or (len(icesat_rh50) <= 1):
                continue

            if np.all(np.isnan(icesat_lon) | np.isnan(icesat_lat) | np.isnan(icesat_ground)):
                continue

            if 'NASADEM' in dem_dataset:
                dem_hgt, dem_slope = get_nasadem_elevation_at_ll(file, icesat_lat, icesat_lon, return_slope=True, nasadem_type=nasadem_type)
                if nasadem_type == 'merged':
                    dem_egm = get_egm_values_at_ll(icesat_lat, icesat_lon)
                ...
                if os.path.isfile(srtm_file):
                    dem_hgt, dem_slope = get_nasadem_elevation_at_ll(srtm_file, icesat_lat, icesat_lon, return_slope=True)
                else:
                ...
                if os.path.isfile(tdx_file):
                    ...
                    dem_hgt, dem_slope = get_raster_at_ll_no_subset(tdx_raster, icesat_lat, icesat_lon, return_slope=True)
                else:
                ...
                if os.path.isfile(cop_file):
                    ...
                    dem_hgt, dem_slope = get_raster_at_ll_no_subset(cop_raster, icesat_lat, icesat_lon, return_slope=True)
                    dem_egm = get_egm_values_at_ll(icesat_lat, icesat_lon, egm='08') # COP-30 uses EGM2008 as vertical datum
                    dem_hgt += dem_egm
                else:
            ...
            # Compare DEM to ICESat-2 ground for bare areas:
            if not forested_flag:
                icesat_hgt = icesat_ground
            else:
                icesat_hgt = icesat_ground + icesat_rh50

            diff = dem_hgt - icesat_hgt
            diff_ground = dem_hgt - icesat_ground
            ...
            if forested_flag:
                ind_keep = (ind_keep
                            ...
                            & (icesat_h_canopy > min_h_canopy)
                            & (icesat_forest_cover <= max_forest_cover)
                            & (icesat_forest_cover >= min_forest_cover)
                            & (icesat_toc_roughness > 0)
                            & (icesat_hansen_gain == 0)
                            & (icesat_hansen_loss == 0)
                            & (icesat_canopy_photons > 0)
                            & (icesat_glims == 0)
                            & (icesat_photons >= min_segment_photons)
                            & (icesat_msw <= max_msw_flag)
                            & (icesat_layer_flag == 0)
                            & (icesat_snr > min_snr)
                            & (icesat_pekel < 5)
                            & (icesat_snowcover == 1) # only include land without snow
                            & (icesat_landcover != 15) # exclude permanent snow
                            & (icesat_landcover != 11)
                            & (icesat_landcover != 0)) # exclude water
            else:
                ind_keep = (ind_keep
                            & (dem_hgt > -32766)
                            & (dem_slope < max_slope_dem)
                            & (icesat_h_canopy < max_h_canopy)
                            & (icesat_ground_uncertainty < max_te_uncertainty)
                            & (icesat_forest_cover <= max_forest_cover)
                            & (icesat_forest_cover >= min_forest_cover)
                            & (icesat_hansen_gain == 0)
                            & (icesat_glims == 0)
            ...
            if smart_orbit_filter:
                # Any orbits with absolute bias > 10 m?
                orbits_eligible_for_filter = np.array([], dtype='int32')
                unique_orbits = np.unique(icesat_end_orbit)
                for num, orbit in enumerate(unique_orbits):
                    ind_orbit = (icesat_end_orbit == orbit)
                    if np.any(ind_keep & ind_orbit):
                        orbit_rmse = np.sqrt(np.nanmean(diff[ind_keep & ind_orbit]**2))
                        if not forested_flag and orbit_rmse > 10:
                            orbits_eligible_for_filter = np.append(orbits_eligible_for_filter, orbit)
                        elif forested_flag and orbit_rmse > np.nanmedian(icesat_h_canopy[ind_keep]):
                            orbits_eligible_for_filter = np.append(orbits_eligible_for_filter, orbit)
                ...
                if len(orbits_eligible_for_filter) > 0:
                    ...
                    for num, orbit in enumerate(orbits_eligible_for_filter):
                        ind_not_orbit = (icesat_end_orbit != orbit)
                    ...
                    try:
                        ...
                        for orbit_to_remove in orbits_eligible_for_filter[ind_rmse_order]:
                            ind_not_orbit = (icesat_end_orbit != orbit_to_remove)
            ...
            # But due to the volume of data, it is hard to implement that for ICESat-2.  But I may want to revisit this (though I'm not sure if it makes a large difference--probably not?).
            if not forested_flag and np.any(ind_keep):
                ind_outlier = (icesat_hgt < -1000) | (icesat_lat > 60) | (diff < np.nanpercentile(diff[ind_keep], percentile_threshold[0])) | (diff > np.nanpercentile(diff[ind_keep], percentile_threshold[1]))
            else:
                ind_outlier = (icesat_hgt < -1000) | (icesat_lat > 60) | (np.abs(diff) > 1000)
            ...
            tile_stats[-1, 6] = np.nanmean(icesat_rh50[ind_keep])
            tile_stats[-1, 7] = np.nanmean(icesat_h_canopy[ind_keep])
            tile_stats[-1, 8] = np.nanmean(icesat_canopy_openness)

            diff_vector = np.append(diff_vector, diff[ind_keep])
            diff_ground_vector = np.append(diff_ground_vector, diff_ground[ind_keep])
            rh50_vector = np.append(rh50_vector, icesat_rh50[ind_keep])
            rh75_vector = np.append(rh75_vector, icesat_rh75[ind_keep])
            hcanopy_vector = np.append(hcanopy_vector, icesat_h_canopy[ind_keep])
    
    return tile_clat, tile_clon, tile_stats, diff_vector, diff_ground_vector, rh50_vector, rh75_vector, hcanopy_vector
...
for dataset_str in dataset_batch:
    outpath = os.path.join(root_path, '{}_validation_icesat2_{}_{}/'.format(dataset_str, icesat_str, version_str))
    ...
    if 'bareshots' == icesat_str:
        ...
        min_h_canopy = -999
        max_h_canopy = 5.0 # only ICESat-2 points with canopy height less than this value will be included
        max_toc_roughness = np.inf
        max_te_uncertainty = np.inf
        min_segment_photons = 0
        min_snr = 1.1
        max_msw_flag = 2
        smart_orbit_filter = False
        ...
        max_slope_dem = 5.0 # no slope threshold for forested analysis # ACTUALLY: testing slope filter for vegetation results
        min_hansen_fc = 30
        max_hansen_fc = np.inf # no forest cover limit for forested analysis
        min_h_canopy = 3.0
        max_h_canopy = np.inf
        max_toc_roughness = np.inf
        max_te_uncertainty = np.inf
    ...
    hcanopy_vector_global = np.array([], dtype='float32')
    ...
    for i, continent in enumerate(continents):
        ...
        tile_clon_file = os.path.join(outpath, continent+'_'+icesat_str+'_tile_clon.npy')
        tile_stats_file = os.path.join(outpath, continent+'_'+icesat_str+'_tile_stats.npy')
        ...
        if os.path.isfile(tile_clat_file) and os.path.isfile(tile_clon_file) and os.path.isfile(tile_stats_file) and not overwrite_icesat_tile_stats:
            ...
            if forested_flag:
                ...
                with h5py.File(h5_veg_continent_output_file, 'r') as hf_veg:
                    ...
                    hcanopy_vector_global = np.append(hcanopy_vector_global, hcanopy_vector)
        else:
            tile_clat, tile_clon, tile_stats, diff_vector, diff_ground_vector, rh50_vector, rh75_vector, hcanopy_vector = get_tile_level_stats(
                    file_pattern, dataset_str, os.path.join(icesat2_path,continent), forested_flag,
                    max_slope_dem, min_hansen_fc, max_hansen_fc,
                    min_h_canopy=min_h_canopy, max_h_canopy=max_h_canopy,
                    max_toc_roughness=max_toc_roughness, max_te_uncertainty=max_te_uncertainty,
                    min_segment_photons=min_segment_photons, max_msw_flag=max_msw_flag,
                    min_snr=min_snr, smart_orbit_filter=smart_orbit_filter,
                    min_tile_samples=min_tile_samples,
                    overwrite_hansen_mask=overwrite_hansen_mask,
                    overwrite_glims_mask=overwrite_glims_mask,
            ...
            if forested_flag:
                ...
                with h5py.File(h5_veg_continent_output_file, 'w') as hf_veg:
                    ...
                    hf_veg.create_dataset('rh100', data=hcanopy_vector)
        ...
        if not skip_plots:
            ...
            hist(tile_stats_filt[:,0], xlim=bias_bounds_hist, xname='Bias of {} Tiles vs. ICESat-2 (m)'.format(dataset_str), numbins=40, units='m', plotmean=False,
                            showrms=False, savefile=os.path.join(figurepath,'{}_Hist_{}_minus_ICESat2_Tiles_Bias.png'.format(continent, dataset_str)))

            hist(tile_stats_filt[:,2], xlim=rmse_bounds_hist, xname='RMSE of {} Tiles vs. ICESat-2 (m)'.format(dataset_str), numbins=40, units='m', plotmean=False,
                            showrms=False, savefile=os.path.join(figurepath,'{}_Hist_{}_minus_ICESat2_Tiles_RMSE.png'.format(continent, dataset_str)))

            ind_valid = tile_stats[:,3] > 10
            tile_clat_filt = tile_clat[ind_valid]
            tile_clon_filt = tile_clon[ind_valid]
            tile_stats_filt = tile_stats[ind_valid, :]
            hist(np.log10(tile_stats_filt[:,3]), xlim=(1, 5), xname='log$_{10}$'+' of Number of ICESat-2 Samples Within Each {} Tile (m)'.format(dataset_str), numbins=40, plotmean=False,
                            showrms=False, savefile=os.path.join(figurepath,'{}_Hist_{}_minus_ICESat2_Tiles_Num.png'.format(continent, dataset_str)))
            plt.close('all')
            ...
            if continent != 'Islands':
                ...
                saveroot_basemap_tile_stats = os.path.join(figurepath,'{}_Basemap_{}_minus_ICESat2_Tiles'.format(continent, dataset_str))
                ...
                plot_basemap(continent, tile_clat_filt, tile_clon_filt, np.log10(tile_stats_filt[:,3]), vmin=1, vmax=5,
                                   cmap='viridis', cbar_label=r'log$_{10}$ of Number of ICESat-2 Samples', patch_size=patch_size,
                ...
                if forested_flag:
                    ...
                    density(hcanopy_vector, diff_ground_vector, xname='ICESat-2 RH100 (m)', yname='{} - ICESat-2 Ground (m)'.format(dataset_str),
                            units='m', lognorm=False, xlim=[0, 60], ylim=[-10, 50], fitline=True, showstats=False, simline=False,
                            savefile=os.path.join(figurepath,'{}_Density_{}_Ground_Diff_vs_ICESat2_RH98.png'.format(continent, dataset_str)))
                    plt.close('all')

                    density(hcanopy_vector, diff_ground_vector, xname='ICESat2 RH100 (m)', yname='{} - ICESat-2 Ground (m)'.format(dataset_str),
                            units='m', lognorm=True, xlim=[0, 60], ylim=[-10, 50], fitline=True, showstats=False, simline=False,
                            savefile=os.path.join(figurepath,'{}_LogDensity_{}_Ground_Diff_vs_ICESat2_RH98.png'.format(continent, dataset_str)))
                    plt.close('all')

                    density(rh50_vector, diff_ground_vector, xname='ICESat2 RH50 (m)', yname='{} - ICESat-2 Ground (m)'.format(dataset_str),
                            units='m', lognorm=False, xlim=[0, 60], ylim=[-10, 50], fitline=True, showstats=False, simline=False,
                            savefile=os.path.join(figurepath,'{}_Density_{}_Ground_Diff_vs_ICESat2_RH50.png'.format(continent, dataset_str)))
                    plt.close('all')

                    density(rh50_vector, diff_ground_vector, xname='ICESat2 RH50 (m)', yname='{} - ICESat-2 Ground (m)'.format(dataset_str),
                            units='m', lognorm=True, xlim=[0, 60], ylim=[-10, 50], fitline=True, showstats=False, simline=False,
                            savefile=os.path.join(figurepath,'{}_LogDensity_{}_Ground_Diff_vs_ICESat2_RH50.png'.format(continent, dataset_str)))
                    plt.close('all')
                    ...
                    plot_basemap(continent, tile_clat_filt, tile_clon_filt, tile_stats_filt[:,6], vmin=0, vmax=15,
                                       cmap='viridis', cbar_label='Mean ICESat-2 RH50 (m)', patch_size=patch_size,
                                       title='RMSE of {} Tiles vs. ICESat2  \n for {} Areas'.format(dataset_str, forested_str),
                    ...
                    plot_basemap(continent, tile_clat_filt, tile_clon_filt, tile_stats_filt[:,7], vmin=0, vmax=30,
                                       cmap='viridis', cbar_label='Mean ICESat-2 RH100 (m)', patch_size=patch_size,
                                       title='RMSE of {} Tiles vs. ICESat2  \n for {} Areas'.format(dataset_str, forested_str),
                    ...
                    plot_basemap(continent, tile_clat_filt, tile_clon_filt, tile_stats_filt[:,8], vmin=0, vmax=5,
                                       cmap='viridis', cbar_label='Mean ICESat-2 Canopy Openness (m)', patch_size=patch_size,
                                       ds_factor=1, savefile=saveroot_basemap_tile_stats+'_MeanCanopyOpenness.png')
                    plt.close('all')

                    hist(tile_stats_filt[:,5], xlim=(0, 30), xname='Vegetation Bias of {} Tiles vs. ICESat-2 Ground (m)'.format(dataset_str), numbins=40, units='m', plotmean=False,
                                    showrms=False, savefile=os.path.join(figurepath,'{}_Hist_{}_vs_ICESat2_Vegetation_Bias.png'.format(continent, dataset_str)))

                    hist(tile_stats_filt[:,6], xlim=(0, 30), xname='Mean ICESat-2 RH50 in NASADEM Tile (m)'.format(dataset_str), numbins=40, units='m', plotmean=False,
                                    showrms=False, savefile=os.path.join(figurepath,'{}_Hist_Mean_ICESat2_RH50.png'.format(continent)))

                    hist(tile_stats_filt[:,7], xlim=(0, 60), xname='Mean ICESat-2 RH100 in NASADEM Tile (m)'.format(dataset_str), numbins=40, units='m', plotmean=False,
                                    showrms=False, savefile=os.path.join(figurepath,'{}_Hist_Mean_ICESat2_RH98.png'.format(continent)))

                    hist(tile_stats_filt[:,8], xlim=(0, 5), xname='Mean ICESat-2 Canopy Openness in NASADEM Tile (m)'.format(dataset_str), numbins=40, units='', plotmean=False,
                                    showrms=False, savefile=os.path.join(figurepath,'{}_Hist_Mean_ICESat-2_Canopy_Openness.png'.format(continent)))
    ...
    if plot_global and not skip_plots:
        print('Plotting {} vs. ICESat-2 Global Stats...'.format(dataset_str, continent))
        # Global Maps
        continent = 'Global'

        h5_veg_output_file = os.path.join(outpath, 'vegetation_analysis_output.h5')
        if forested_flag and os.path.isfile(h5_veg_output_file) and not overwrite_icesat_tile_stats:
            ...
            with h5py.File(h5_veg_output_file, 'r') as hf_veg:
                ...
                hcanopy_vector_global = hf_veg['rh100'][:]
        elif forested_flag:
            # Save files so we can compare vegetation bias between NASADEM compared to other datasets.
            with h5py.File(h5_veg_output_file, 'w') as hf_veg:
                ...
                hf_veg.create_dataset('rh100', data=hcanopy_vector_global)
        else:
        ...
        hist(tile_stats_global_filt[:,0], xlim=bias_bounds_hist, xname='Bias of {} Tiles vs. ICESat-2 (m)'.format(dataset_str), numbins=40, units='m', plotmean=False,
                        showrms=False, savefile=os.path.join(figurepath,'{}_Hist_{}_minus_ICESat_Tiles_Bias.png'.format(continent, dataset_str)))

        hist(tile_stats_global_filt[:,2], xlim=rmse_bounds_hist, xname='RMSE of {} Tiles vs. ICESat-2 (m)'.format(dataset_str), numbins=40, units='m', plotmean=False,
                        showrms=False, savefile=os.path.join(figurepath,'{}_Hist_{}_minus_ICESat_Tiles_RMSE.png'.format(continent, dataset_str)))

        hist(np.log10(tile_stats_global_all[:,3]), xlim=(1, 5), xname='log$_{10}$'+' of Number of ICESat-2 Samples Within {} Tile'.format(dataset_str), numbins=40, plotmean=False,
        ...
        plot_basemap(continent, tile_clat_global_all, tile_clon_global_all, np.log10(tile_stats_global_all[:,3]), vmin=1, vmax=5,
                           cmap='viridis', cbar_label=r'log$_{10}$ of Number of ICESat-2 Samples', patch_size=patch_size,
                           title='Number of ICESat Samples Within Each ICESat-2 Tile \n for {} Areas'.format(dataset_str, forested_str),
                           ds_factor=1, savefile=saveroot_basemap_tile_stats+'_Num.png')
        plt.close('all')
        ...
        if forested_flag:
            ...
            for lower_bin, upper_bin in zip(rh100_bin_edges[0:-1], rh100_bin_edges[1:]):
                ind_bin = (hcanopy_vector_global >= lower_bin) & (hcanopy_vector_global <= upper_bin)
            ...
            if np.any(ind_keep):
                ...
                hcanopy_vector_global = hcanopy_vector_global[ind_keep]
            ...
            ind_fit = ((hcanopy_vector_global > 3.0) & np.isfinite(rh50_vector_global)
                      & np.isfinite(rh75_vector_global) & np.isfinite(hcanopy_vector_global)
            ...
            rmse_rh100 = np.sqrt(np.nanmean(np.square(diff_ground_vector_global[ind_fit] - hcanopy_vector_global[ind_fit])))

            print('Veg. Stats for Paper ({} vs. ICESat-2):'.format(dataset_str))
            print('')
            print('RH100:')
            fit_slope, fit_intercept, fit_r2, zifit_slope, zifit_r2 = do_fits_for_paper(hcanopy_vector_global[ind_fit], diff_ground_vector_global[ind_fit])
            ...
            # Plot of Height Differences vs. RH100
            density(hcanopy_vector_global, diff_ground_vector_global, xname='ICESat-2 RH100 (m)', yname='{} - ICESat-2 Ground (m)'.format(dataset_str),
                    units='m', lognorm=True, xlim=[0, 60], ylim=[-5, 60], fitline=True, showstats=False, simline=False,
                    savefile=os.path.join(figurepath,'Global_LogDensity_{}_Ground_Diff_vs_ICESat2_RH98.png'.format(dataset_str)))

            density(hcanopy_vector_global, diff_ground_vector_global, xname='ICESat-2 RH100 (m)', yname='{} - ICESat-2 Ground (m)'.format(dataset_str),
                    units='m', lognorm=False, xlim=[0, 60], ylim=[-5, 60], fitline=True, showstats=False, simline=False,
                    savefile=os.path.join(figurepath,'Global_Density_{}_Ground_Diff_vs_ICESat2_RH98.png'.format(dataset_str)))
            plt.close('all')
            ...
            plot_basemap(continent, tile_clat_global_filt, tile_clon_global_filt, tile_stats_global_filt[:,6], vmin=0, vmax=15,
                               cmap='viridis', cbar_label='Mean ICESat-2 RH50 In Tile (m)', patch_size=patch_size,
                               title='RMSE of {} Tiles vs. ICESat-2  \n for {} Areas'.format(dataset_str, forested_str),
                               ds_factor=1, savefile=saveroot_basemap_tile_stats+'_MeanRH50.png')
            plt.close('all')

            plot_basemap(continent, tile_clat_global_filt, tile_clon_global_filt, tile_stats_global_filt[:,7], vmin=0, vmax=30,
                               cmap='viridis', cbar_label='Mean ICESat-2 RH100 In Tile (m)', patch_size=patch_size,
                               title='RMSE of {} Tiles vs. ICESat-2  \n for {} Areas'.format(dataset_str, forested_str),
                               ds_factor=1, savefile=saveroot_basemap_tile_stats+'_MeanRH100.png')
            plt.close('all')

            plot_basemap(continent, tile_clat_global_filt, tile_clon_global_filt, tile_stats_global_filt[:,8], vmin=0, vmax=10,
                               cmap='viridis', cbar_label='Mean ICESat-2 Canopy Openness In Tile (m)', patch_size=patch_size,
                               ds_factor=1, savefile=saveroot_basemap_tile_stats+'_MeanCanopyOpenness.png')
            plt.close('all')


            hist(tile_stats_global_filt[:,5], xlim=(0, 30), xname='Vegetation Bias of {} Tiles vs. ICESat-2 Ground (m)'.format(dataset_str), numbins=40, units='m', plotmean=False,
                            showrms=False, savefile=os.path.join(figurepath,'{}_Hist_{}_vs_ICESat2_Vegetation_Bias.png'.format(continent, dataset_str)))

            hist(tile_stats_global_filt[:,6], xlim=(0, 30), xname='Mean ICESat-2 RH50 (m)'.format(dataset_str), numbins=40, units='m', plotmean=False,
                            showrms=False, savefile=os.path.join(figurepath,'{}_Hist_Mean_ICESat2_RH50.png'.format(continent)))

            hist(tile_stats_global_filt[:,7], xlim=(0, 60), xname='Mean ICESat-2 RH100 (m)'.format(dataset_str), numbins=40, units='m', plotmean=False,
                            showrms=False, savefile=os.path.join(figurepath,'{}_Hist_Mean_ICESat2_RH100.png'.format(continent)))

            hist(tile_stats_global_filt[:,8], xlim=(0, 10), xname='Mean ICESat-2 Canopy Openness (m)'.format(dataset_str), numbins=40, units='', plotmean=False,
                            showrms=False, savefile=os.path.join(figurepath,'{}_Hist_Mean_ICESat2_Canopy_Openness.png'.format(continent)))
            plt.close('all')


        geotiff_basename = os.path.join(figurepath,'Global_{}_v_ICESat2_Tile_Stats'.format(dataset_str))
        create_geotiff_stats(tile_clat_global_filt, tile_clon_global_filt, tile_stats_global_filt, geotiff_basename)

        output_shpfile = os.path.join(figurepath,'Global_{}_v_ICESat2_Tile_Stats.shp'.format(dataset_str))
        create_shp_stats(tile_clat_global_filt, tile_clon_global_filt, tile_stats_global_filt, output_shpfile)


print(' -- validation_icesat2.py Finished! --')